...
if not players:
    st.warning("Sem dados analíticos de elenco. Rode: `python main.py --team fortaleza --build`")
    st.stop()
...
active_df["prod_total"] = ((active_df["prod_p90"] * active_df["minutes"]) / 90.0).round(2)

total_market_val = active_df["market_value_eur"].dropna().sum()
avg_age = active_df["age"].dropna().mean()
...
c1.metric("Atletas Ativos", len(active_df))
c2.metric("Valor Total do Elenco", _fmt_eur(total_market_val))
c3.metric("Média de Idade", f"{avg_age:.1f} anos" if not pd.isna(avg_age) else "N/A")
c4.metric(
